...
class MPCOnmiControl
{
    public:
        //@ brief 默认构造函数与析构函数
        MPCOnmiControl(double sample_time,int Np,int Nc);
        virtual ~ MPCOnmiControl();

       //@ brief 设置参考轨迹
       void setRefTraj(vector<trajOnmi> & ref_traj);

       //@ brief 更新轨迹上离机器人实际位置最近的点
       void updateNearestRefState();

        //@ brief 更新当前机器人的状态变量（即当前位姿）
        void updateState(trajOnmi & current_pos);

        //@ brief 计算当前位置和对应轨迹参考点的误差
        void computeErrors(trajOnmi & traj_ref);
        ...
        bool getFirstControl(double& vy,double& vw);
        bool getRealControl(vector<double>& real_vy,vector<double>& real_vw);

        bool isGoalReached();

        //存放实际控制量
        vector<double> real_control_vy_;
        vector<double> real_control_vw_;
        vector<double> pf;
        vector<double> preal;
        vector<double> dy;
        vector<double> dy2;
        ...
        //存放实际控制量偏差
        vector<double> real_control_vy_offset_;
        vector<double> real_control_vw_offset_;
        vector<double> ddduuu;
    protected:
        //@brief 计算两个点之间的距离
        double calculateDistance(trajOnmi pos1,trajOnmi pos2); 

        //@ brief 更新轨迹上离机器人实际位置最近的点
        void updateNearestRefState(trajOnmi& traj_ref_pos,int & traj_ref_index);  
    ...
    private:
        ...
        //状态矩阵用到的参考量
        trajOnmi nearest_ref_traj_pos_;
        ...
        //约束参数
        //转角单个方向的范围
        double wheel_single_vw_max_degree_ = 0.0;
        //最大转角转速
        double wheel_single_vw_max_vel_ = 0.0;
        //最大速度
        double vy_max_ = 0.0;
        //最大纵向加速度
        double ay_max_ = 0.0;
